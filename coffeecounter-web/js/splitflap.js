    ...
    var lastchild = new Firebase("https://amber-torch-2593.firebaseio.com");
    ...
    var hourly_sum = null;
        function updateTime() {
            ...
            var offset = -8;
            serverTime = new Date( new Date().getTime() + offset * 3600 * 1000).toUTCString().replace( / GMT$/, "" );
            $('#time').html(serverTime);

            //if(serverTime.substring(serverTime.length-8, serverTime.length)=="15:45:50"){
                myFirebaseRef.child("coffee").orderByKey().on("value", function(snapshot){
                    var object_coffee = snapshot.val();
                    var last_key = Object.keys(object_coffee)[Object.keys(object_coffee).length-1];
                    hourly_sum = object_coffee[last_key]['hourly'];
                    //console.log(JSON.stringify(hourly_sum));
                });
                if(hourly_sum!=null){
                    var date = serverTime.toString().substring;
                    var date_obj = {}; 
                    date_obj[$.datepicker.formatDate('yy-mm-dd', new Date())] = hourly_sum;
    ...
    myFirebaseRef.child("coffee").orderByKey().limitToLast(1).on("child_added", function(snapshot){
        lastchild = snapshot.val();
        
        var total = pad(lastchild['total'],4);
        var daily = pad(lastchild['daily'],3);
        var timestamp = lastchild['timestamp'];
        var hourly = lastchild['hourly'];
        var id = lastchild['id'];
        console.log(hourly);
        $("#print input#total").val(total);
        $("#print input#daily").val(daily);
        $("#print input#timestamp").val(timestamp);
        $("#print input#hourly").val(JSON.stringify(hourly)); //Turn JSON into strings
        //getData('2015-03-03');
        $("#print input#id").val(id);
        
        $("#print").submit();
    });

    function getData(timeString){
        myFirebaseRef.child("coffee").orderByKey().on("value", function(snapshot){
            var object_coffee = snapshot.val();
            var date_key = Object.keys(object_coffee)[0];
            var hourly_sum = object_coffee[date_key][timeString];
            //console.log(JSON.stringify(hourly_sum));
            $("#print input#hourly").val(JSON.stringify(hourly_sum)); //Turn JSON into strings
            //$("#print").submit();
        });
    }
